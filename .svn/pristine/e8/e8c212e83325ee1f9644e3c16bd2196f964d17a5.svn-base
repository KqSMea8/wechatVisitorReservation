    ...
    <flexbox>
      <flexbox-item>
        <x-button type="primary"
                  @click.native="showPosition()">预约</x-button>
      </flexbox-item>
...
import { isVisType } from '../../utils/common.js'
...
export default {
  ...
  methods: {
        ...
        success: function (res) {
          ...
          vm.$wechat.uploadImage({
            ...
            success: function (res) {
              ...
              console.log('faceId  ', serverId)
              vm.faceId = serverId
              console.log('上传人脸成功')
              console.log('this.faceId', vm.faceId)
            }
          })
        }
      })
      ...
      vm.$wechat.chooseImage({
        ...
        success: function (res) {
          ...
          vm.$wechat.uploadImage({
            ...
            success: function (res) {
              var serverId = res.serverId // 返回图片的服务器端ID
              console.log('zIdentifyId  ', serverId)
              vm.zIdentifyId = serverId
              console.log('身份证正面上传成功')
              console.log('zIdentifyId', vm.zIdentifyId)
            }
          })
        }
      })
      ...
      vm.$wechat.chooseImage({
        ...
        success: function (res) {
          ...
          vm.$wechat.uploadImage({
            ...
            success: function (res) {
              var serverId = res.serverId // 返回图片的服务器端ID
              console.log('fIdentifyId  ', serverId)
              vm.fIdentifyId = serverId
              console.log('身份证反面上传成功')
              console.log('fIdentifyId', vm.fIdentifyId)
            }
          })
        }
      })
    ...
    showPosition (position) {
      ...
      let num = this.dataNum[0]
      this.startTime = this._timeFormat(vm.userInfo.startTime)
      this.endTime = this._timeFormat(vm.userInfo.endTime)
      let visTypeStr = this._visTypeFormat(vm.visitorType)
      reservationInfo.append('trueName', this.userInfo.visTrueName)
      reservationInfo.append('telephone', this.userInfo.visTelephone)
      reservationInfo.append('resTelephone', this.userInfo.resTelephone)
      reservationInfo.append('startTime', this.startTime)
      reservationInfo.append('endTime', this.endTime)
      reservationInfo.append('visitorNum', num.toString())
      reservationInfo.append('visitorType', visTypeStr)
      reservationInfo.append('carNumber', this.userInfo.carNumber)
      reservationInfo.append('faceId', vm.faceId)
      reservationInfo.append('zIdentifyId', this.zIdentifyId)
      reservationInfo.append('fIdentifyId', this.fIdentifyId)
    ...
    _timeFormat (val) {
      ...
      console.log('时间格式化')
      // throw new Error('_timeFormat 方法传入的必须是一个数组')
    },
    _visTypeFormat (val) {
      console.log('访客类型', val)
      if (val instanceof Array) {
      ...
      // throw new Error(' _visTypeFormat 方法传入的必须是一个数组')
    },
    _isVisType () {
      let vm = this
      if (!isVisType()) {
